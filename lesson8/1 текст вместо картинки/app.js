...
buttonEls.forEach(e => e.addEventListener("click", handleClick));
...
function handleClick(clickedButtonEvent) {
    ...
    let buttonText = clickedButtonEvent.target.innerText;
...
function showMoreText(card) {
    ...
    let text = card.img.currentSrc;
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.wrap.insertAdjacentHTML("afterbegin",`<div class="desc">${text}</div>`);
    //let divEl = document.createElement("div");
    //divEl.innerText = text;
    //divEl.classList.add("desc");

    card.wrap.insertBefore(card.wrap.querySelector(".desc"), card.img);
